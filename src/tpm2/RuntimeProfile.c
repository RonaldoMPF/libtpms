...
#define STATE_FORMAT_LEVEL_UNKNOWN 0 /* JSON didn't provide StateFormatLevel; this is only
    ...
    {
        /* The default profile replaces the NULL profile. */
	.name = "default",
	/* A NULL command profile means that all currently implemented commands
	 * can be used. When the state is written, the current set of commands
	 * are written out and upon restart only this set of commands can be used.
	 * A non-NULL profile locks the user into this set of commands.
	 */
	.commandProfile    = NULL,
		  ...
		  )
{
    ...
    retVal = GetParametersFromJSON(json, &profileName, &stateFormatLevel,
				   &algorithmsProfile);
    if (retVal != TPM_RC_SUCCESS)
	return retVal;
